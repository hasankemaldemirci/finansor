...
const secureStateStorage: StateStorage = {
  getItem: (name: string): string | null => {
    try {
      ...
      if (value) {
        ...
        if (value && typeof value === 'object' && !value.state && !value.version) {
      ...
      console.error(`[SecureStorageAdapter] GetItem failed for: ${name}`, error);
  ...
  setItem: (name: string, value: string): void => {
    try {
      ...
      console.error(`[SecureStorageAdapter] SetItem failed for: ${name}`, error);
