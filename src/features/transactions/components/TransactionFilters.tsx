      ...
      <div className="flex items-center justify-between">
        ...
        {hasActiveFilters && (
          <div className="pt-2 text-sm text-muted-foreground">
            ...
            {filters.type !== 'all' && <span className="mr-2">📊 {filters.type === 'income' ? 'Gelir' : 'Gider'}</span>}
            {filters.category !== 'all' && <span className="mr-2">🏷️ Kategori seçili</span>}
            {filters.dateRange !== 'all' && <span className="mr-2">📅 Tarih filtresi</span>}
            {(filters.minAmount !== undefined || filters.maxAmount !== undefined) && <span className="mr-2">💰 Tutar aralığı</span>}
          </div>
        )}
    </div>
  );
